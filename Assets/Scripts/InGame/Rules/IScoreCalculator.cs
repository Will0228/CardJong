...
namespace CardJong.InGame.Rules
{
    ...
    public interface IScoreCalculator
    {
        ...
        WinResult Evaluate(int winnerSeat, int loserSeat, Card winningCard);

        /// <summary>上がり時の席ごとの点数増減。合計は 0 になる。</summary>
        int[] CalculateWinDeltas(WinResult win);

        /// <summary>流局時（ノーテン罰符）の席ごとの点数増減。合計は 0 になる。</summary>
        int[] CalculateDrawGameDeltas(IReadOnlyList<int> tenpaiSeats);
    }
}
